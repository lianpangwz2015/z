...
include 'config.php';
$pindao="风行电影";
$url="http://tv6080.pronvod.com/dy/fengxing.php";
$url=getUrl($url);
preg_match_all('|p=(.*?)"|',$url,$r);
foreach($r[1] as $k=>$v){
$url="http://tv6080.pronvod.com/dy/fengxing.php?p=".$v;
$list.=getUrl($url);
}
preg_match_all('|id=(.*?)" label="(.*?)"|',$list,$r);
foreach($r[1] as $k=>$v){
$lrc=iconv("UTF-8", "GB2312//IGNORE", $r[2][$k]);

$liebiao.='<a href="dyfx.php?id='.$v."&title=".urlencode($lrc).'" target="_self" hidefocus="true" onMouseOver="ahover(\''.$v.'_1\')" onMouseOut="alink(\''.$v.'_1\')" id="'.$v.'">'.$lrc .'</a>';
...
$zb="iframe.php?id=http%3a%2f%2fm.fun.tv%2fmplay%2f%3fmid%3d".$_GET['id']."&t=40&l=10&w=1298&h=800&label=".urlencode($pindao);
}
...
<div id="halist" style="display:block;background-color: #000000;height:100%;overflow:scroll;SCROLLBAR-FACE-COLOR: #444; SCROLLBAR-HIGHliGHT-COLOR: #999; SCROLLBAR-SHADOW-COLOR: #222; SCROLLBAR-3DliGHT-COLOR: #000; SCROLLBAR-ARROW-COLOR: #ccc; SCROLLBAR-TRACK-COLOR: #666; SCROLLBAR-DARKSHADOW-COLOR: #111;">
  <ul id="menu">
<li id="li_1">
  <h1 class="lm_1"><a href="javascript:OpenChanel(1);" class="list_name" hidefocus="true">风行电影 <font style="color:#99cc00;font-size:8px">|||||</font></a></h1>
