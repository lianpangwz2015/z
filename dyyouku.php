...
include 'config.php';
$pindao="优酷电影";
$url="http://tv6080.pronvod.com/dy/youku.php?u=-30";
$url=getUrl($url);
preg_match_all('|p=(.*?)"|',$url,$r);
foreach($r[1] as $k=>$v){
$url="http://tv6080.pronvod.com/dy/youku.php?p=".$v;
$list.=getUrl($url);
}

preg_match_all('|src="(.*?)" label="(.*?)"|',$list,$r);
foreach($r[1] as $k=>$v){
$lrc=iconv("UTF-8", "GB2312//IGNORE", $r[2][$k]);

$liebiao.='<a href="dyyouku.php?id='.$v."&title=".urlencode($lrc).'" target="_self" hidefocus="true" onMouseOver="ahover(\''.$v.'_1\')" onMouseOut="alink(\''.$v.'_1\')" id="'.$v.'">'.$lrc .'</a>';
...
$zb="/cmp/dsjplay.php?src=".$_GET['id']."&type=youku&label=".urlencode($pindao);
}
...
<div id="halist" style="display:block;background-color: #000000;height:100%;overflow:scroll;SCROLLBAR-FACE-COLOR: #444; SCROLLBAR-HIGHliGHT-COLOR: #999; SCROLLBAR-SHADOW-COLOR: #222; SCROLLBAR-3DliGHT-COLOR: #000; SCROLLBAR-ARROW-COLOR: #ccc; SCROLLBAR-TRACK-COLOR: #666; SCROLLBAR-DARKSHADOW-COLOR: #111;">
  <ul id="menu">
<li id="li_1">
  <h1 class="lm_1"><a href="javascript:OpenChanel(1);" class="list_name" hidefocus="true">优酷电影 <font style="color:#99cc00;font-size:8px">|||||</font></a></h1>
