...
include 'list2.php';
$pindao="电视剧";
$url="http://tv6080.pronvod.com/dsj/sohu.php?u=-20";
$url=getUrl($url);
preg_match_all('|p=(.*?)"|',$url,$r);
foreach($r[1] as $k=>$v){
$url="http://tv6080.pronvod.com/dsj/sohu.php?p=".$v;
$list.=getUrl($url);
}
preg_match_all('|v=(.*?)" label="(.*?)"|',$list,$r);
foreach($r[1] as $k=>$v){
$lrc=iconv("UTF-8", "GB2312//IGNORE", $r[2][$k]);

$liebiao.='<a href="dsj.php?id='.$v."&title=".urlencode($lrc).'" target="_self" hidefocus="true" onMouseOver="ahover(\''.$v.'_1\')" onMouseOut="alink(\''.$v.'_1\')" id="'.$v.'">'.$lrc .'</a>';
$baiduurl.='http://www.letvlive.com/dsj.php?id='.$v.'&title='.urlencode($lrc).',';
}

$id=$_GET['id'];
if($id){
$pindao=urldecode($_GET['title']);
$zb="/cmp/dsjplay.php?lists=http://tv6080.pronvod.com/dsj/sohu.php?v=".$_GET['id']."&title=".urlencode($pindao);
$url="http://tv6080.pronvod.com/dsj/sohu.php?v=".$_GET['id'];
$url=getUrl($url);
preg_match_all('|src="(.*?)" label="(.*?)"|',$url,$r);
foreach($r[1] as $k=>$v){
$l=iconv("UTF-8", "GB2312//IGNORE", $r[2][$k]);
$p=urlencode($pindao.$l);
$playurl="/dsjplay.php?src=".$v."&type=sohu&title=".$p;
$ll.='<a href="'.$playurl.'" target="_blank"  >'.$l.'</a>&nbsp;&nbsp;';
